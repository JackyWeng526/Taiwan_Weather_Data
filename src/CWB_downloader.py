...
class CWB_downloader:
    def __init__(self, station_number, date_start, date_end):
        ...
        self.st_name, self.st_altitude = self.CWB_station_name_altitude()
        self.time_period = self.download_time_period()

    def CWB_station_name_altitude(self):
        ...
        st_name = CWB_meta_df.loc[CWB_meta_df["站號"]==str(self.stn), "站名"].values[0]
        st_altitude = CWB_meta_df.loc[CWB_meta_df["站號"]==str(self.stn), "海拔高度(m)"].values[0]
        return st_name, st_altitude
    ...
    def data_downloader(self):
        ...
        altitude = self.st_altitude
        for d in range(date_delta):
            date = date_start + datetime.timedelta(days=d)
            html_df = pd.read_html(F"https://e-service.cwb.gov.tw/HistoryDataQuery/DayDataController.do?command=viewMain&station={stn}&stname={st_name}&datepicker={date}&altitude={altitude}")[1]
